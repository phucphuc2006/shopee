<%@page import="model.User, model.Address, java.util.List" %>
...
<head>
    ...
    <title>Địa Chỉ | Shopee Việt Nam</title>
...
<body>
    <div class="app">
        ...
        <% if (successMsg != null) { %><div class="profile-toast success show" id="profileToast"><i class="fas fa-check-circle"></i><span><%= successMsg %></span></div><% } %>
        <% if (errorMsg != null) { %><div class="profile-toast error show" id="profileToast"><i class="fas fa-exclamation-circle"></i><span><%= errorMsg %></span></div><% } %>
                ...
                <div class="profile-sidebar">
                    ...
                    <div class="profile-sidebar-menu" id="sidebarMenu">
                        <div class="profile-sidebar-user">
                            ...
                            <div><div class="profile-sidebar-name"><%= acc != null ? acc.getFullName() : "" %></div><a href="${pageContext.request.contextPath}/user/account/profile" class="profile-sidebar-edit"><i class="fas fa-edit"></i> Sửa Hồ Sơ</a></div>
                        </div>
                        <div class="profile-menu-section">
                            ...
                            <a href="${pageContext.request.contextPath}/user/account/address" class="profile-menu-item active"><i class="fas fa-map-marker-alt"></i> Địa Chỉ</a>
                ...
                <div class="profile-main">
                    <div class="account-card">
                        <div class="address-header">
                            <div class="account-card-title">Địa Chỉ Của Tôi</div>
                            <button class="address-add-btn" onclick="openAddModal()"><i class="fas fa-plus"></i> Thêm Địa Chỉ Mới</button>
                        </div>
                            ...
                            <div class="address-item">
                                <div class="address-info">
                                    <div class="address-name-phone">
                                        <%= addr.getFullName() %> <span><%= addr.getPhone() %></span>
                                    </div>
                                    <div class="address-detail"><%= addr.getAddressDetail() %></div>
                                    <% if (addr.isDefault()) { %>
                                        <div class="address-default-badge">Mặc định</div>
                                    <% } %>
                                </div>
                                <div class="address-actions">
                                    <div>
                                        <button class="address-action-link" onclick="openEditModal(<%= addr.getId() %>, '<%= addr.getFullName().replace("'", "\\'") %>', '<%= addr.getPhone() %>', '<%= addr.getAddressDetail().replace("'", "\\'") %>', <%= addr.isDefault() %>)">Sửa</button>
                                        <% if (!addr.isDefault()) { %>
                                        <form action="${pageContext.request.contextPath}/user/account/address" method="post" style="display:inline;">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="id" value="<%= addr.getId() %>">
                                            <button type="submit" class="address-action-link delete" onclick="return confirm('Bạn có chắc muốn xóa địa chỉ này?')">Xóa</button>
                                        </form>
                                        <% } %>
                                    </div>
                                    <% if (!addr.isDefault()) { %>
                                    <form action="${pageContext.request.contextPath}/user/account/address" method="post">
                                        <input type="hidden" name="action" value="setDefault">
                                        <input type="hidden" name="id" value="<%= addr.getId() %>">
                                        <button type="submit" class="address-set-default-btn">Thiết lập mặc định</button>
                                    </form>
                                    <% } %>
                                </div>
                            </div>
                        <%  }
                            ...
                            <div class="account-empty">
                                <div class="account-empty-icon"><i class="fas fa-map-marker-alt"></i></div>
                                <div class="account-empty-text">Bạn chưa có địa chỉ nào</div>
                            </div>
                        <% } %>
                    </div>
                </div>
    ...
    <div class="account-modal-overlay" id="addressModal">
        <div class="account-modal">
            <button class="account-modal-close" onclick="closeModal()"><i class="fas fa-times"></i></button>
            <div class="account-modal-title" id="modalTitle">Thêm Địa Chỉ Mới</div>
            <form action="${pageContext.request.contextPath}/user/account/address" method="post" id="addressForm">
                <input type="hidden" name="action" id="modalAction" value="add">
                <input type="hidden" name="id" id="modalId" value="">
                <div class="account-form-row">
                    <div class="account-form-label">Họ và tên</div>
                    <div class="account-form-value"><input type="text" name="fullName" id="modalFullName" placeholder="Nhập họ và tên" required></div>
                </div>
                <div class="account-form-row">
                    <div class="account-form-label">Số điện thoại</div>
                    <div class="account-form-value"><input type="tel" name="phone" id="modalPhone" placeholder="Nhập số điện thoại" required></div>
                </div>
                <div class="account-form-row">
                    <div class="account-form-label">Địa chỉ cụ thể</div>
                    <div class="account-form-value"><input type="text" name="addressDetail" id="modalAddress" placeholder="Số nhà, đường, phường, quận, thành phố" required></div>
                </div>
                <div class="account-form-row">
                    <div class="account-form-label"></div>
                    <div class="account-form-value">
                        <label style="display:flex; align-items:center; gap:6px; font-size:14px; cursor:pointer;">
                            <input type="checkbox" name="isDefault" id="modalDefault" style="accent-color:#ee4d2d;"> Đặt làm địa chỉ mặc định
                        </label>
                    </div>
                </div>
                <div class="account-modal-actions">
                    <button type="button" class="account-cancel-btn" onclick="closeModal()">Hủy</button>
                    <button type="submit" class="account-save-btn">Hoàn thành</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(id, name, phone, address, isDefault) {
            ...
            document.getElementById('modalDefault').checked = isDefault;
            document.getElementById('addressModal').classList.add('show');
        }
        function closeModal() {
